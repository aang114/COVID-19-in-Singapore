...
def get_y_true_and_y_pred(files, neural_net, grc_row_number, denormalization_vector):
    ...
    return y_true, y_pred, week_numbers
...
def plot_graph(plot_type, y_true, y_pred, dates, graph_plot_directory):
    ...
    if plot_type == PlotType.TrainedModel.ground_truth_versus_prediction:
        ...
        title = 'Error 404'
        if graph_plot_directory == File.training_accuracy_for_weekly_cases_png:
            title = 'Training Cases (Ground Truth vs Prediction)'
        elif graph_plot_directory == File.testing_accuracy_for_weekly_cases_png:
...
with open(File.trained_model_for_weekly_cases, 'rb') as f:
    net = pickle.load(file=f)

with open(File.train_files_for_weekly_cases, 'rb') as f:
    train_files = pickle.load(file=f)

with open(File.test_files_for_weekly_cases, 'rb') as f:
...
with open(File.model_accuracy_for_weekly_cases_json, 'w') as f:
...
plot_graph(plot_type=PlotType.TrainedModel.ground_truth_versus_prediction,
           ...
           dates=None,
           graph_plot_directory=File.training_accuracy_for_weekly_cases_png)
...
plot_graph(plot_type=PlotType.TrainedModel.ground_truth_versus_prediction,
           ...
           dates=None,
           graph_plot_directory=File.testing_accuracy_for_weekly_cases_png)



for row_number, grc_name in enumerate(dfc.index):

    training_y_true, training_y_pred, training_dates = get_y_true_and_y_pred(files=train_files,
                                                                             neural_net=net,
    ...
    plot_graph(plot_type=PlotType.TrainedModel.date_versus_ground_truth_and_prediction_for_training_and_testing_dataset,
               ...
               graph_plot_directory=Folder.grc_accuracy_folder_for_weekly_cases + '{}.png'.format(grc_name))